...
print "<style>img{border: 1px; border-style: solid; width: auto;}</style>";
...
chop(@fromfile);
close(DFILE);
$ip_ok=1; #для доступа отовсюду $ip_ok=1
foreach $line (@fromfile)
    {
     ($line_username, $line_ip) =split(/;/, $line, 2);
     #print $line_username, $line_ip, $ip_ok, "\n";
     #if ($remote_address eq $line_ip  and $username eq $line_username) {$ip_ok=1}
     if ($username eq $line_username) {$ip_ok=1}
    }
if ($ip_ok==1)
        {
        ...
        if ($number == -1)
                    ...
                    $number_of_line=0;
                    $path_in="../public_html/testing/t_2/"; $file_in=$path_in."test.txt";
                    ...
                    open(user_file,">../public_html/testing/results/t_2/".$username.".vop");
                    ...
                    open(user_file,">>../public_html/testing/results/t_2/".$username.".tim");
                    print user_file "$^T\n";
                    close(user_file);
                    #Создание файла username.txt
                    open(user_file,">>../public_html/testing/results/t_2/".$username.".txt");
                    close(user_file);
                    #Создание файла username.res
                    open(user_file,">>../public_html/testing/results/t_2/".$username.".res");
                    close(user_file);
        ...
        if ($quest_quantity -$number >=0)
                ...
                open(UFILE,"../public_html/testing/results/t_2/".$username.".vop");
                @fromfile=<UFILE>;
                chop(@fromfile);
                close(UFILE);
                ...
                $name_out="../public_html/testing/results/t_2/".$username.".txt";
                open (OUT, ">>",$name_out) || die"mistake-$name_out";
                print OUT "\n",$answer;
                print OUT "\n\n------";
                print OUT "\n",$number;
                print OUT "\n", $remote_address,", ",$year,".",$mon, ".", $mday, ",  ", $hour, ":", $min, ":", $sec, ",  ", $first_number; 
                print OUT "\n",$picture_img;
                print OUT "\n",$question;
                print OUT "\n",$corr_answer;
                close (OUT);
...
#                print "<br><TEXTAREA name='01_answer_01' value='' rows=8 cols=100 ></TEXTAREA>\n";
                print "<br><br><input type =text name='01_answer_01' value='' size=120>\n";
...
#                print "<input type = hidden  name = '01_question_01' value='$question'>\n";
                ...
                $example=$path_in.$example_picture;
                $example="http://198.211.123.137:7600/public_html/testing/perl.gif";
                $example_1="http://198.211.123.137:7600/public_html/testing/hse30.png";
                print "\n<pre>

Ответ преобразовать в одну строку
Использовать  кодировку utf-8
Дождаться завершения текущей операции и появления нового вопроса.
Head, body, table и их родительские элементы существуют, поэтому фрагмент таблицы нужно начинать с tr, 
форму с form, параграф с p, список с ul или ol  и аналогично с элементами div, img  и другими.
При работе с таблицами ориентироваться на структуру таблицы, представленную на рисунке
При создании форм использовать правила умолчания и данные должны отправляться на http://198.211.123.137:7633/cgi-bin/form_action.py
Для ссылок и для изображений  использовать абсолютную адресацию.
Если нужно изображение, можно использовать  $example
или $example_1
Количество вопросов - $quest_quantity_enter. На лишний вопрос можно не отвечать.
...
#                print FILE_OUT $question,"    ", $corr_answer,"    ",$answer,"    ",$picture_img, "\n";
        ...
        #-------------------------------------------------
          if ($quest_quantity -$number < 100 )
               { 
                $name_in="../public_html/testing/results/t_2/".$username.".txt";
                open FILE_IN,$name_in or die "mistake-$name_in" ;
                $name_out="../public_html/testing/results/t_2/".$username.".res";
                open (FILE_OUT, ">",$name_out) || die"mistake-$name_out";
                $n_i = -1;
                $corr_numb=0;
                while ($pri = <FILE_IN>)
...
#для запрета на просмотр строку выше расскомменировать  
                        {
                            $name_in="../public_html/testing/results/t_2/".$username.".res";
...
#                                print"\n",$n_bl;
                                #print "\n",$n_bl,$pri;
                                if ($n_bl == 2)     {($n_writed)= split(/\n/,$pri,2);}                             
                                if ($n_bl == 3)     {($n_time)= split(/\n/,$pri,2);}                                  
                                if ($n_bl == 4)     {($picture_img)= split(/\n/,$pri,2);}                                
        ...
        #                                ($question, $corr_answer, $answer,$picture_img) =split(/\t/, $pri, 4);
                                        print "<p>&nbsp;</p>";
                                        print "\n\n<h3>Вопрос:$n_writed; Логин: $username; IP,дата,время: $n_time</h3>"; # Номер
                                        print "\n<img src=$picture_img border=1>";
                                        print "\n<p>Вопрос:<br> $question\n"; # Вопрос
                                        print "\n<table width=700 cellpadding=0 border=1 bgcolor=white>";
                                        print "<tr><td width=33% ></td><td></td><td  width=33% ></td></tr>"; #разметка ячеек
...
{
#print "Content-type: text/html\n\n";
print "Access denied:\n<br>",$username, ";", $remote_address,"\n<br>Строку выше скопировать и отправить преподавателю";
...
#    print user_file $remote_address,"\n", @pairs,"\n";
#close(user_file);
print "\n<br>После согласования строки с преподавателем, обновитесь, кликнув F5";
}
