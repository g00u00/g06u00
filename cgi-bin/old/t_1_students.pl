...
$quest_quantity_enter=6;   #!!!!___Ввести количество вопросов
$quest_quantity = $quest_quantity_enter+1;  



$my_ip="89.189.16.120";
$my_ip="194.190.181.148";# IP к-ра, с которого доступ на тестирование Вышка
$control_ip="89.189.18.137"; # IP к-ра, с которого производится проверка тестов
$my_ip="89.189.16.120";
$my_ip="194.190.181.147";# IP к-ра, с которого доступ на тестирование Вышка
$my_ip=$remote_address;#если внутри Вышки - строку раскомментировать
$control_ip=$my_ip;#если разрешается просмотр - строку раскомментировать
...
#				print "<input type = hidden  name = '01_question_01' value='$question'>\n";
				...
				print "</n 
<p>Ответ записать в одну строку<br>Строку для таблицы создавать в контексте все й таблицы и следует  начинать с tr  в угловых скобках 
...
#				   if (($quest_quantity -$number < -1) and ($remote_address eq $control_ip) )  для запрета на просмотр строку рскомменировать  
						{
		...
		#								($question, $corr_answer, $answer,$picture_img) =split(/\t/, $pri, 4);
										...
										if ($answer eq $corr_answer)#
											{$corr_quantity=$corr_quantity+1;
												...
												else {print "<br><font color='Red'><B>Ответ неверен </B></font>";}
										print "<HR>";
									}
								}
						}
