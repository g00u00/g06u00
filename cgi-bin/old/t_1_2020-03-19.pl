...
#для доступа отовсюду две строку выше раскомменировать, строку ниже закоментировать 
if ($remote_address eq "95.79.179.27"
    or $remote_address eq "95.79.179.128"
    or $remote_address eq "176.59.99.48000"
    or  $remote_address eq "46.229.141.80")
#"46.229.141.88" - 201; ???  - 301 ; ??? 146
		{
		$query_str =~ tr/+/ /;
...
<br>Для ссылок и для изображений использовать абсолютную адресацию на Интернет-ресурс.
...
#								print "\n",$n_bl,$pri;
		...
		#								($question, $corr_answer, $answer,$picture_img) =split(/\t/, $pri, 4);
										...
										print "\n<table width=500px border=1 bgcolor=white>";
                                        print "<tr><td border=0></td><td border=0></td><td border=0></td></tr>"; #разметка ячеек
                                        print "<tr><td border=0 colspan=3>Правильный ответ:</td></tr>"; # Правильный ответ
										print "\n$corr_answer\n"; # Правильный ответ
                                        print "<tr><td border=0 colspan=3>Ответ студента:</td></tr>"; # Правильный ответ
										print "\n$answer\n"; # Ответ пользователя
										print "\n </font></u></p></td></tr></table></div>";  #  - подчистка за неправильным ответом - если студент что-то не закрыл
										print "\n </font></font></u></u></p></p></td></td></tr></tr></table></table></div></div>";  #  - подчистка за неправильным ответом - если студент что-то не закрыл
