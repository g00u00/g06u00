...
#для доступа отовсюду две строку выше раскомменировать, строку ниже закоменнтировать 
#if ($remote_address eq "85.140.3._214"  or $remote_address eq "46.229.141._88" or $remote_address eq "194.190.181._222")
		{
		$query_str =~ tr/+/ /;
		...
		if ($quest_quantity -$number >=0)
				...
				$file_out="../public_html/testing/results/t_0/".$username.".txt";
		...
		#-------------------------------------------------
		  if ($quest_quantity -$number < 0 )
			   { 
				...
				$file_out="../public_html/testing/results/t_0/".$username.".res";
				open (input_file, ">",$file_out) || die"mistake-$file_out";
				$n_i = -1;
				$corr_numb=0;
				while ($pri = <FILE_IN>)
