...
$quest_quantity_enter=5;   #!Ввести количество вопросов - на два вопроса меньше списка
...
#if ($remote_address eq "46.229.141.80"  or $remote_address eq "46.229.141.88" or $remote_address eq "194.190.181.2")
		{
		$query_str =~ tr/+/ /;
...
#				print "<input type = hidden  name = '01_question_01' value='$question'>\n";
				...
				print "</n 
<p>Ответ записать в одну строку<br>Строку для таблицы создавать в контексте всей таблицы и следует  начинать с tr  в угловых скобках 
...
#								print "\n",$n_bl,$pri;
		...
		#								($question, $corr_answer, $answer,$picture_img) =split(/\t/, $pri, 4);
										...
										print "\n<p>Правильный ответ:\n<table  bgcolor='silver' width='400' border=1>\n $corr_answer <tr> <td>0.</td> <td>0.</td>  <td>0.</td></tr>\n</table>\n"; # Правильный ответ
										print "\n<p>Ответ студента: \n<table  bgcolor='silver' width='400' border=1>\n $answer <tr> <td>0.</td> <td>0.</td>  <td>0.</td></tr>\n</table>\n"; # Ответ пользователя
										print "\n </font></u></p></td></tr></table></font></u></p></td></tr></table>\n";  #  - подчистка за неправильным ответом - если студент что-то не закрыл
